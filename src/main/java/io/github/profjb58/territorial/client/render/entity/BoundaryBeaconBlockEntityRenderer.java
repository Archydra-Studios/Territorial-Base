...
public class BoundaryBeaconBlockEntityRenderer implements BlockEntityRenderer<BoundaryBeaconBlockEntity> {

    public static final Identifier BEAM_TEXTURE = new Identifier("textures/entity/beacon_beam.png");
    public static final int MAX_BEAM_HEIGHT = 1024;

    @Override
    public void render(BoundaryBeaconBlockEntity beaconBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
        long l = beaconBlockEntity.getWorld().getTime();
        List<BoundaryBeaconBlockEntity.BeamSegment> list = beaconBlockEntity.getBeamSegments();
        int k = 0;

        for(int m = 0; m < list.size(); ++m) {
            BoundaryBeaconBlockEntity.BeamSegment beamSegment = list.get(m);
    ...
    @Override
    public boolean rendersOutsideBoundingBox(BoundaryBeaconBlockEntity beaconBlockEntity) {
        return true;
    }
    ...
    @Override
    public boolean isInRenderDistance(BoundaryBeaconBlockEntity beaconBlockEntity, Vec3d vec3d) {
